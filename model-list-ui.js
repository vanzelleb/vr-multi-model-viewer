...
export function renderDownloadedModels() {
  ...
  models.forEach((m, idx) => {
    ...
    // Only show model name, size, and thumbnail in the top row
    li.innerHTML = `
      <div class="model-thumb-row" style="display:flex;align-items:center;gap:1rem;">
        <img src="${m.thumbnail || ''}" alt="${m.name}" style="width:64px;height:64px;object-fit:cover;border-radius:6px;" onerror="this.style.display='none'" />
